...
Every winter, emergency departments across the US fill up with respiratory illness cases. Every summer, they empty out. The question we set out to answer: can we predict *how bad* a given day will be based on environmental data available in advance — temperature, air pollution, humidity — before patients actually show up?

The target variable is `pct_respiratory`: the daily percentage of all US emergency department visits classified as Acute Respiratory Illness (ARI) by the CDC's National Syndromic Surveillance Program. This ranges from about 7% in summer to 27% in winter — a 3.7× seasonal swing. We treat this as a supervised regression problem: given today's national environmental readings, predict today's (or the near-future) respiratory ED burden.

If you can predict it reliably, hospitals can pre-staff before a surge, public health officials can issue targeted advisories, and policymakers can quantify which pollutants matter most and with what delay.
...
The pipeline runs across two notebooks: `data_profiling.ipynb` inspects each raw dataset and documents every issue found without modifying any data; `data_cleaning.ipynb` then executes exactly the decisions made during profiling. Nothing in cleaning was improvised — every operation traces back to a documented finding.

### PM2.5

8,306 readings had negative values (physically impossible for a concentration) caused by sensor calibration drift and were dropped. For extreme high values, a hard threshold would have been wrong: the 192 readings above 200 µg/m³ correspond to the June–September 2023 Canadian wildfire smoke events that blanketed the US. These are real, validated measurements. A 99.5th percentile cap at the station level preserved these events while trimming only instrument artifacts at the very tail of the distribution.

### Temperature

One reading of −1,177.67°F — below absolute zero and therefore impossible — was a firmware error code from a malfunctioning sensor. 79 readings above 140°F were surface radiant heat artifacts from sensors sited on hot rooftops. Physical limits (drop below −80°F, drop above 140°F) were used rather than percentile caps because the errors here are identifiable by physics, not just by being statistically extreme.

### Humidity

The raw EPA humidity file mixes two completely different measurements in the same rows: Relative Humidity (in percent, 0–100%) and Dew Point (in degrees Fahrenheit, which can be negative). A naive average of these columns produces meaningless numbers. The raw profile showed a minimum of −25%, which looked like a bad humidity reading but was actually a dew point value in Fahrenheit. The fix was filtering to `Parameter Name == "Relative Humidity"` rows before any aggregation.

### Respiratory (CDC NSSP)

The CDC file contains rows for many different pathogens (COVID, Flu, RSV, etc.) and many geographic levels (individual states, regions, national). We filtered to `pathogen == "ARI"` and `geography == "United States"` using exact string matching. ARI (Acute Respiratory Illness) is the broadest respiratory category in NSSP — it includes all respiratory ED visits regardless of specific pathogen. Using ARI alongside individual pathogen rows would double-count the same visits. The target variable showed no invalid values, no missing days, and a meaningful weekly oscillation (slightly different ED utilization on weekends vs weekdays) that was kept as real behavioral signal rather than smoothed out.

### Merge

All six cleaned datasets were joined on date using an inner join. The study period is bounded by the CDC data on the early end (starts September 25, 2022) and the temperature data on the late end (ends October 31, 2025). Since no dataset had missing days within this window, the inner join preserved all 1,133 days with zero missing values.
...
The first thing that jumps out from plotting all variables over time is how closely temperature tracks respiratory illness. Cold winters push respiratory ED visits up to 24–27% of all visits; warm summers bring that down to 7–9%. The relationship is consistent across all three years in the dataset. NO₂ follows a similar seasonal pattern for a different reason: more combustion-based heating and traffic in winter increases nitrogen dioxide, and higher NO₂ is associated with more respiratory illness.
...
PM2.5 shows almost no correlation at any lag (r ≈ −0.14). This is not evidence that PM2.5 is harmless — it's a seasonal confounding artifact. The biggest PM2.5 events in the dataset (the 2023 wildfire smoke plumes) occurred in summer, exactly when respiratory illness is at its annual minimum. The two signals are out of seasonal phase and partially cancel each other, masking any real causal effect.

### Feature correlation ranking

![Correlation ranking](outputs/figures/eda_03_feature_correlation_ranking.png)

This chart compares same-day correlation vs best-lag correlation for each feature. For temperature and humidity, the two bars are nearly identical — lag doesn't help. For NO₂ and ozone, the lagged correlation is meaningfully stronger. PM2.5 barely registers in either case.
...
The evaluation design is the most consequential decision in the modeling notebook.
...
The fold-by-fold CV results show a clear seasonal pattern: folds 2 and 4 (fall/winter test sets) return positive R² across all models; folds 1, 3, and 5 (spring/summer test sets) return negative R². This isn't the models breaking — it's them correctly learning the distribution they were trained on and failing when tested on seasons they haven't seen enough of. The leave-last-year-out holdout avoids this by ensuring the test period covers all seasons.
